...
//ClassImp(DSRhoPDFRConstraint)

DSRhoPDFRConstraint::DSRhoPDFRConstraint(const char *name, const char *title, const char *_type,
                   RooAbsReal& _tht,
                   RooAbsReal& _thb,
                   RooAbsReal& _phit,
                   RooAbsReal& _dt,
...
DSRhoPDFRConstraint::DSRhoPDFRConstraint(const DSRhoPDFRConstraint& other, const char* name) :
...
Double_t DSRhoPDFRConstraint::evaluate() const
{
    Int_t phiw_sign = 1;
    ...
    if(type == 1 || type == 2)
    {
        At2 = at*at*((1+r*r)+(1-r*r)*cos(Bfreq*dt)+2*r*sin(phiw_sign*phiw-st)*sin(Bfreq*dt));
        Ap2 = ap*ap*((1+r*r)+(1-r*r)*cos(Bfreq*dt)-2*r*sin(phiw_sign*phiw-sp)*sin(Bfreq*dt));
        A02 = a0*a0*((1+r*r)+(1-r*r)*cos(Bfreq*dt)-2*r*sin(phiw_sign*phiw-s0)*sin(Bfreq*dt));

        Ap0r =    ap0r*(1+r*r*cos(sp-s0))+ap0i*r*r*sin(sp-s0)+\
                          (ap0r*(1-r*r*cos(sp-s0))-ap0i*r*r*sin(sp-s0))*cos(Bfreq*dt)-\
                          (ap0r*(r*sin(phiw_sign*phiw-sp)+r*sin(phiw_sign*phiw-s0))+\
                           ap0i*(r*cos(phiw_sign*phiw-sp)-r*cos(phiw_sign*phiw-s0)))*sin(Bfreq*dt);

        A0ti =    a0ti*(1-r*r*cos(s0-st))+a0tr*r*r*sin(s0-st)+\
                      (a0ti*(1+r*r*cos(s0-st))-a0tr*r*r*sin(s0-st))*cos(Bfreq*dt)-\
                      (a0ti*(r*sin(phiw_sign*phiw-s0)-r*sin(phiw_sign*phiw-st))-\
                       a0tr*(r*cos(phiw_sign*phiw-s0)+r*cos(phiw_sign*phiw-st)))*sin(Bfreq*dt);

        Apti =    apti*(1-r*r*cos(sp-st))+aptr*r*r*sin(sp-st)+\
                          (apti*(1+r*r*cos(sp-st))-aptr*r*r*sin(sp-st))*cos(Bfreq*dt)-\
                          (apti*(r*sin(phiw_sign*phiw-sp)-r*sin(phiw_sign*phiw-st))-\
                           aptr*(r*cos(phiw_sign*phiw-sp)+r*cos(phiw_sign*phiw-st)))*sin(Bfreq*dt);
    }
    ...
    if(type == 3 || type == 4)
    {
        At2 = at*at*((1+r*r)+(1-r*r)*(-1)*cos(Bfreq*dt)+2*r*sin(phiw_sign*phiw-st)*(-1)*sin(Bfreq*dt));
        Ap2 = ap*ap*((1+r*r)+(1-r*r)*(-1)*cos(Bfreq*dt)-2*r*sin(phiw_sign*phiw-sp)*(-1)*sin(Bfreq*dt));
        A02 = a0*a0*((1+r*r)+(1-r*r)*(-1)*cos(Bfreq*dt)-2*r*sin(phiw_sign*phiw-s0)*(-1)*sin(Bfreq*dt));

        Ap0r =    ap0r*(1+r*r*cos(sp-s0))+ap0i*r*r*sin(sp-s0)+\
                          (ap0r*(1-r*r*cos(sp-s0))-ap0i*r*r*sin(sp-s0))*(-1)*cos(Bfreq*dt)-\
                          (ap0r*(r*sin(phiw_sign*phiw-sp)+r*sin(phiw_sign*phiw-s0))+\
                           ap0i*(r*cos(phiw_sign*phiw-sp)-r*cos(phiw_sign*phiw-s0)))*(-1)*sin(Bfreq*dt);

        A0ti =    a0ti*(1-r*r*cos(s0-st))+a0tr*r*r*sin(s0-st)+\
                      (a0ti*(1+r*r*cos(s0-st))-a0tr*r*r*sin(s0-st))*(-1)*cos(Bfreq*dt)-\
                      (a0ti*(r*sin(phiw_sign*phiw-s0)-r*sin(phiw_sign*phiw-st))-\
                       a0tr*(r*cos(phiw_sign*phiw-s0)+r*cos(phiw_sign*phiw-st)))*(-1)*sin(Bfreq*dt);

        Apti =    apti*(1-r*r*cos(sp-st))+aptr*r*r*sin(sp-st)+\
                          (apti*(1+r*r*cos(sp-st))-aptr*r*r*sin(sp-st))*(-1)*cos(Bfreq*dt)-\
                          (apti*(r*sin(phiw_sign*phiw-sp)-r*sin(phiw_sign*phiw-st))-\
                           aptr*(r*cos(phiw_sign*phiw-sp)+r*cos(phiw_sign*phiw-st)))*(-1)*sin(Bfreq*dt);
    }
...
Int_t DSRhoPDFRConstraint::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const
{
...
Double_t DSRhoPDFRConstraint::analyticalIntegral(Int_t code, const char* rangeName) const
{
...
//    Double_t a0ti = a0*at*sin(-a0a+ata);
    ...
    case 1: // Int[g,{tht,thb,phit,dt}]
        if(type == 1 || type == 2)
            return 128.*PI/9.*(1/Bgamma+Bgamma/(Bfreq*Bfreq+Bgamma*Bgamma)+(ap*ap*r*r+a0*a0*r*r+at*at*r*r)*(1/Bgamma-Bgamma/(Bfreq*Bfreq+Bgamma*Bgamma)));
        else
            return 128.*PI/9.*(1/Bgamma-Bgamma/(Bfreq*Bfreq+Bgamma*Bgamma)+(ap*ap*r*r+a0*a0*r*r+at*at*r*r)*(1/Bgamma+Bgamma/(Bfreq*Bfreq+Bgamma*Bgamma)));
